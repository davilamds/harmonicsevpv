...
input.time_steps = 144; %>>>>>>Steps of the daily simulation (1 day = 1440 steps of 1min)
%% Start up the DSS
...
enable_pvsystems=0; %0 disabled 1 enabled (case 1) 2 enabled (case 2)
enable_vehicles=0;
...
DSSText.Command = 'Set Mode = daily stepsize = 10m number = 1'; %>>>>>>Defines simulatins parameters
...
%  clear time_simulation g1 g2 g3 g4 g5 g6
    ...
    figure;
    plot(TIEMPO ,voltaje_cliente_1(:,7));
    hold on
    
    title('Load Current (I1)')
    xlabel('Time [Hour]')
    ylabel('Current [I]')
    xlim([0 24]) 
    legend('Load 349')
